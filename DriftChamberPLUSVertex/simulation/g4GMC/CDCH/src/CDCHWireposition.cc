...
CDCHWireposition::CDCHWireposition(Bool_t newFile, const char *WireDataFile )
{

  //
  // CDCHWireposition default constructor
  //

  fwiredata= new CDCHWiredata();
  if (newFile) {
    wirefile = new TFile(WireDataFile,"RECREATE");
    trwdata = new TTree("WireData","WireData");
    trwdata->Branch("WireDataMatrix","CDCHWiredata",&fwiredata);
    ...
    if(intStat == 0) {
      ...
      for(int i=0;i< trwdata->GetEntries();i++){
        fWireDataAll[i]=new CDCHWiredata();
...
//_____________________________________________________________________________
void CDCHWireposition::AddSuperLayer(Int_t nCelLayer, Int_t nwire){
...
void CDCHWireposition::InsertAlfaEpsilon(Int_t iCel, Float_t alfa, Float_t eps){
  fwiredata->epsilon[iCel]=eps;
  fwiredata->alfa[iCel]=alfa;
}

void CDCHWireposition::InsertRadius(Int_t iCel, Float_t rad){
  fwiredata->radius_z0[iCel]=rad;
}

void CDCHWireposition::InsertWireMatrix(Int_t nCelL, Int_t nw, TGeoHMatrix *matrix){
  ((TObjArray*)fwiredata->PosMatrix->At(nCelL))->AddAt( matrix, nw );
}

void CDCHWireposition::WriteData(){
  wirefile->cd();
  trwdata->Write();
}

void CDCHWireposition::FillData(){
  trwdata->Fill();
}



 void CDCHWireposition::SelectWire(Int_t SupLayer, Int_t CelLayer, Int_t Wire){
...
void CDCHWireposition::SelectWireDet(ULong_t det){
...
Double_t CDCHWireposition::DistFromWireCenter(Double_t *global){
...
Double_t CDCHWireposition::DistFromWire(Double_t *global){
...
void CDCHWireposition::WirePosAtEndcap(Float_t *right, Float_t *left){
...
void CDCHWireposition::WirePosAtZ(Float_t z, Float_t *pos){
